...
# MAGIC %run ../Utils/prepare-lab-environment_2
...
# MAGIC
# MAGIC If your data is already in the cloud - you can simply read it from ADLS 
...
# MAGIC %sql 
# MAGIC -- If you're using SQL you might want to check the data with SQL 
# MAGIC SELECT * FROM JustAView
...
dbutils.fs.ls(f"{datasets_location}")
...
checkpoint_path = f"{datasets_location}/checkpoints/sales"

(spark.readStream
  .format("<format>")
  .option("cloudFiles.format", "<format>")
  .option("cloudFiles.schemaLocation", checkpoint_path)
  .load(f"{datasets_location}/sales") #
  .writeStream
